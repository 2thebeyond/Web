...
if (isset($_POST['user_id']) && isset($_POST['user_nick']) && isset($_POST['password1']) && isset($_POST['password2']))
{
	...
	if (empty($uid)){
		header("location: ../../views/account/register_view.php?error=이메일이 비어있어요.");
		exit();
	} else if (empty($unick)){
		header("location: ../../views/account/register_view.php?error=닉네임이 비어있어요.");
		exit();
	} else if (empty($pwd)){
		header("location: ../../views/account/register_view.php?error=비밀번호가 비어있어요.");
		exit();
	} else if (empty($pwdCheck)){
		header("location: ../../views/account/register_view.php?error=비밀번호 확인이 비어있어요.");
		exit();
	} else if ($pwd !== $pwdCheck){
		header("location: ../../views/account/register_view.php?error=비밀번호가 일치하지 않습니다.");
		exit();
	} else {
		...
		{
			 header("location: ../../views/account/register_view.php?error=이메일 형식이 잘못되었습니다.");
			exit();
		}
		
		if (mysqli_num_rows($id_overlapCheck) > 0){
			header("location: ../../views/account/register_view.php?error=중복되는 아이디가 존재합니다.");
			exit();
		} else if (mysqli_num_rows($nick_overlapCheck) > 0){
			header("location: ../../views/account/register_view.php?error=중복되는 닉네임이 존재합니다.");
			exit();
		} else {
			...
			if($result){
				header("location: ../../views/account/register_view.php?success=성공적으로 가입되었습니다.");
				exit();
			} else {
				header("location: ../../views/account/register_view.php?error=가입에 실패하였습니다.");
				exit();
			}
		}
	}
...
else if (isset($_POST['user_id']) && isset($_POST['password']))
{
	...
	if (empty($uid)){
		header("location: ../../views/account/login_view.php?error=이메일이 비어있어요.");
		exit();
	} else if (empty($pwd)){
		header("location: ../../views/account/login_view.php?error=비밀번호가 비어있어요.");
		exit();
	} else {
		...
		if (mysqli_num_rows($result) === 1){
			...
			if (password_verify($pwd, $hash)){
				...
				header("location: ../../views/profile/mypage_view.php");
				exit();
			} else {
				header("location: ../../views/account/login_view.php?error=로그인에 실패하였습니다.");
				exit();
			} 
		} else {
			header("location: ../../views/account/login_view.php?error=이메일이 잘못되었습니다.");
			exit();
		}
	} 
} 
...
{
	header("location: ../../views/account/login_view.php?error=알 수 없는 오류가 발생하였습니다.");
	exit();
}
?> 